...
// update
if( !empty($_GET['id']) && !empty($_GET['tableName']) && $_GET['role']=='student'){
   $id=legal_input($_GET['id']);
   $tableName=legal_input($_GET['tableName']);
   $query="SELECT status FROM student_profile WHERE id=$id";
   $res= $db->query($query);
   $role=$res->fetch_assoc();
   if($role['status']==1)
   {
...
// insert
if(empty($_GET['id']) && !empty($_GET['name']) && $_GET['name']=='student_profile'){
   extract($_POST);
  if(!empty($full_name) & !empty($password)){
   if($password==$cpassword)
   {
    ...
    if(!empty($data) && !empty($tableName)){
       $insertData=insert_data($data,$tableName);
       if($insertData){
         echo "<span class='success'>Student Profile saved sucessfully</span>";
       }else{
         echo "<span class='fail'>Error!.. check your query</span>";
       }
    }
  ...
  echo "<span class='fail'>Student Profile field is empty</span>";
}
...
// insert data
if(!empty($_GET['name']) && $_GET['name']=='new_student'){
   extract($_POST);
  if(!empty($full_name) && !empty($email) & !empty($password)){
     $data= [
       'full_name' =>legal_input($full_name),
    ...
    if(!empty($data) && !empty($tableName)){
       $insertData=insert_data($data,$tableName);
       if($insertData){
         echo "<span class='success'>New Student saved sucessfully</span>";
       }else{
         echo "<span class='fail'>Error!.. check your query</span>";
       }
    }
...
// update data
if(!empty($_GET['id']) && !empty($_GET['name']) && $_GET['name']=='update_new_student')
{
 ...
 if(!empty($full_name) && !empty($email) & !empty($password)){
  ...
  $updateData=update_data($data,$tableName, $id);
   if($updateData){
    echo "<span class='success'>Student updated sucessfully</span>";
   }else{
    echo "<span class='fail'>Error!.. check your query</span>";
   }
