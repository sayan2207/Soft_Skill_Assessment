...
$(document).on("click", ".studentRole", function (e) {
  var el = $(this);
  var name = $(this).attr("name");
  var id = $(this).attr("rel");
  $.ajax({
    method: "GET",
    url: "scripts/backend-script.php",
    data: { tableName: name, id: id, role: "student" },

    success: function (data) {
      switch (data) {
        case "fas fa-user-alt-slash iconRole":
          el.attr("class", "text-secondary studentRole");
          break;
        case "fas fa-user-alt iconRole":
          el.attr("class", "text-success studentRole");
          break;
      }
...
//======= Insert data through student form==========//
